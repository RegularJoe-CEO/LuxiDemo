...
scripts/hw_details_linux.sh docs/benchmarks/hw_details_gpu.txt || true
PM="docs/benchmarks/torch_gpu_baseline_power.txt"
CSV="docs/benchmarks/torch_gpu_baseline_power.csv"
python3 -m pip install -q --user pynvml >/dev/null 2>&1 || true
python3 scripts/power_linux.py "$DUR" "$PM" &
PM_PID=$!
python3 benchmarks/torch_pipeline_gpu.py --a "$A" --batch-size "$BS" --duration-s "$DUR" --csv "$CSV"
wait "$PM_PID"
# Optional latency CSV
python3 - <<'PY'
import json, time, numpy as np, torch
from pathlib import Path
csv="docs/benchmarks/torch_gpu_baseline.csv"
...
print("Wrote", csv)
PY
python3 benchmarks/summarize_bench.py || true
echo "GPU powered bench files written."
ls -l docs/benchmarks/torch_gpu_baseline*.csv docs/benchmarks/torch_gpu_baseline*.txt docs/benchmarks/xai_integration.md || true

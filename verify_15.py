...
def compute_sha256(results):
    raw = b''.join(struct.pack('<f', v) for v in results)
    return hashlib.sha256(raw).hexdigest()

passed = 0
for expr, values, expected_hash in TESTS:
    resp = requests.post(ENDPOINT, json={"expr":expr,"x":values,"precision":"f32"})
    results = resp.json()
    got_hash = compute_sha256(results)
    if got_hash == expected_hash:
        print(f"✅ {expr}: PASS")
        passed += 1
    else:
        print(f"❌ {expr}: FAIL - got {got_hash[:16]}... expected {expected_hash[:16]}...")

...
pub mod interpreter {
    ...
    fn create_engine() -> Engine {
        ...
        eng.register_fn("erf", |x: f64| libm::erf(x));
        eng.register_fn("erfc", |x: f64| libm::erfc(x));
        eng
    }
    ...
    fn assemble_vars(base: &HashMap<String, f64>, var: Option<(&str, f64)>) -> HashMap<String, f64> {
        let mut vars = base.clone();
        if let Some((name, value)) = var { vars.insert(name.to_string(), value); }
        vars
    }
    ...
    pub fn simd_eval_over_x_inplace(_root: usize, arena: &Arena, fixed: &HashMap<String, f64>, mut xs: Vec<f64>) -> Vec<f64> {
        let len = xs.len();
        if len == 0 { return xs; }
        for j in 0..len { xs[j] = eval_with_var(arena, fixed, "x", xs[j]); }
        xs
    }

    pub fn derivative_with_var(arena: &Arena, base: &HashMap<String, f64>, var: &str, value: f64, h: f64) -> f64 {
    ...
    pub fn gradient(arena: &Arena, base: &HashMap<String, f64>, variables: &[String], h: f64) -> HashMap<String, f64> {
        let mut grads = HashMap::new();
        for var in variables {
            let value = match base.get(var) { Some(v) => *v, None => { grads.insert(var.clone(), f64::NAN); continue; } };
    ...
    pub fn batch_eval_optimized(arena: &Arena, fixed: &HashMap<String, f64>, xs: &[f64]) -> Vec<f64> {
        let n = xs.len();
        let mut results = Vec::with_capacity(n);
        let eng = create_engine();
        let mut scope = Scope::new();
        for (k, v) in fixed.iter() { scope.push_dynamic(k.as_str(), Dynamic::from(*v)); }
        for &x in xs {
            scope.set_value("x", x);
            match eng.eval_with_scope::<Dynamic>(&mut scope, &arena.expr) {
